...
def do_the_magic(config):
    ...
    for f in os.listdir(data_dir):
        if not ".txt" in f:
            continue
        tmp_df = pd.read_csv(
            os.path.join(data_dir, f), delimiter=",", decimal=".", quotechar='"'
        )
    ...
    cal["end_date"] = cal["end_date"].apply(
        lambda x: datetime.datetime.strptime(str(x), "%Y%m%d")
    )
    ...
    servicejourney.to_csv(
        os.path.join(config["out_directory"], "servicejourney.txt"),
        index=False,
        sep=";",
    )
    ...
    stoppoints = pd.read_csv(
        os.path.join(config["out_directory"], "stoppoints.txt"), sep=";"
    )

    sjdf = pd.read_csv(
        os.path.join(config["out_directory"], "servicejourney.txt"), sep=";"
    )
    sp_red = stoppoints[
    ...
    od_matrix["start"] = crossprod.loc[crossprod["ID_x"] != crossprod["ID_y"], :].apply(
        lambda x: ",".join([str(x["Lat_x"]), str(x["Lon_x"])]), axis=1
    )
    od_matrix["destination"] = crossprod.loc[
    ...
    connections[
        ["FromStopID", "ToStopID", "FromLineID", "ToLineID", "MinTransferTime"]
    ].to_csv(
